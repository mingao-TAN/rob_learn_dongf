...
RANGE_OFFFSET = 0.095 # in Meter

# changed by mingao:对激光扫描数据进行下采样
def downsample_scan(scan, factor):
"""
对激光扫描数据进行下采样。
:param scan: 激光扫描数据 (LaserScan)。
:param factor: 下采样因子，每隔多少个点取一个点。
:return: 下采样后的激光扫描数据 (LaserScan)。
"""
new_ranges = []
for i in range(0, len(scan.ranges), factor):
new_ranges.append(scan.ranges[i])
# 更新下采样后的数据
scan.ranges = new_ranges
scan.angle_max = scan.angle_min + (len(new_ranges) - 1) * scan.angle_increment
return scan

# changed by mingao:对scan_camera扫描数据进行下采样
...
global scan_camera
DOWNSAMPLE_FACTOR = 2
scan_camera = downsample_scan(data, DOWNSAMPLE_FACTOR)

rospy.loginfo("scancamera: %s",scan_camera)
...
rospy.loginfo("scan: %s",scan)
# rospy.loginfo("scan: %s",str(lidar_scan))
# 计算相机扫描数据的一半长度
...
# 发布融合后的扫描数据
publisher_scan_fusion.publish(scan)
rospy.loginfo("scan_fusion: %s",scan)
