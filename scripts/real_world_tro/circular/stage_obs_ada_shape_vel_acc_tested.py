...
class StageWorld():
    def __init__(self, beam_num):
        ...
        goal_set1 = np.load('goal_set_nev2.npy')
        robot_set1 = np.load('robot_set_nev2.npy') 
        config_set1 = np.load('config_set_nev2.npy') 
        goal_set = goal_set1
...
#        noise[noise < -0.03] = -0.03
    ...
    def ResetWorld(self,env_no,radius):
        rospy.sleep(4.0)
        self.past_actions = deque(maxlen=2)
        for initial_zero in range(2):
            self.past_actions.append(0)
        ...
        self.radius=radius
        self.robot_range_x1 = self.radius+0.15
        self.robot_range_x2 = self.radius+0.15
        self.robot_range_y = self.radius+0.15
...
#        print(self.self_speed[0])        
    ...
    def set_robot_pose_test(self,i,env_no,robot_no):
        ...
        self.max_action[1] = self.config_initials[robot_no,i,4]
        ...
        #print("action bound is", self.max_action,"acc bound is", self.max_acc)
        self.radius=self.config_initials[robot_no,i,0]
        self.stop_counter = 0.0
        self.crash_stop = False
        self.env=env_no
        self.map_size = self.map_sizes[env_no]
...
#        expand_p4 = np.array(np.matmul(expand_p3,rot_m))
        ...
        if np.min(pool_state[:,2])<=self.radius:
