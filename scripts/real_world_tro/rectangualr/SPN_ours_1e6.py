...
#            length_index=np.random.choice(range(5),p=p)  
            ...
            if length_index==0:
                ...
                while length1+length2+width*2<(length_index+1.0)*0.4 or length1+length2+width*2>=(length_index+2.0)*0.4:
                    length1=np.random.uniform(0.075,0.6)
                    length2=np.random.uniform(0.075,0.6)
                    width=np.random.uniform(0.075,(length2+length1)/2.0)
...
#                    robot_size = env.ResetWorld(env_no)
            ...
            print("train env no. is",env_no,'target distance',round(r2gd,1),'succ rate',mean_rate[length_index],"length1",round(length1,2),"length2",round(length2,2),"width",round(width,2),"max_ep_len",max_ep_len)
            reset = False
...
#                            print(batch['obs1'])
                    ...
                    #                                                trainable_saver.save(sess, '/home/zhw1993/Monocular-Obstacle-Avoidance/D3QN/input_compare/ETH/networks/sac'+str(sac)+'lambda'+str(hyper_exp), global_step=test_time)
                                                    ...
                                                    if shape_no==4:
                                                        ...
                                                        if (test_time) % 1== 0:
                                                            trainable_saver.save(sess, '/home/zhw1993/Monocular-Obstacle-Avoidance/D3QN/paper3_dot/Good_with_backward/networks/configback540GMMdense'+str(sac)+'lambda'+str(hyper_exp), global_step=test_time)
                                                            rospy.sleep(3.0)
                                                    b_test = False
                                                break                                
