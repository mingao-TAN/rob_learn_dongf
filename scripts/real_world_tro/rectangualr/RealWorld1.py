...
#        self.action_table = [[-1.,-1.],[-1.,0.],[-1.,1.],[0.,-1.],[0.,0.],[0.,1.],[1.,-1.],[1.,0.],[1.,1.]]
        ...
        self.max_action = [1.0, np.pi/3]  # 最大线速度和角速度
        self.min_action = [0.0, 0.0]  # 最小线速度和角速度
    ...
    #  -------------- update 724  by mingao : set the robot length and width------------------------  #
        ...
        self.width=0.31  # half width  -> 0.375
...
# ------------------------ change by mingao, 0813 minpool 代码: 将1667个点转为90个，采用分段转换的方法 ------------------------  #
    def min_pool(self, state, num_pools=90):
        ...
        print("Starting min_pool, Input state shape: {}".format(np.shape(state)))
        
        state = np.array(state).flatten()
        
        print("Flattened state shape:  {}".format(np.shape(state)))
        ...
        print("Ending min_pool. Pooled state shape: {}".format(np.shape(pooled_state)))   
        return pooled_state

# ------------------------ change by mingao： 执行一步动作并返回新的状态 ------------------------  #
    def step(self):
        try: 
            print("Starting the step method")
    ...
    #        cur_act[0] = cur_act[0]
        ...
        # ----------- change by shanze, 0816, to check where the error is -------------
        except Exception as e:
            print("Error in step method: {}".format(str(e)))
            import traceback
            traceback.print_exc()
